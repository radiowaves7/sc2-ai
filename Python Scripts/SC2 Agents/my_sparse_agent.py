...
# https://github/MorvanZhou/Reinforcment-learning-with-tensorflow
class QLearningTable:
    ...
    def learn(self, state, action, reward, statenew):
        self.check_state_exist(statenew)
        self.check_state_exist(state)
        q_predict = self.q_table.ix[state, action]
        if statenew != "terminal":
            q_target = reward + self.gamma * self.q_table.ix[statenew, :].max()
        else:
            q_target = reward
...
class SparseAgent(base_agent.BaseAgent):
    def __init__(self):
        ...
        if os.path.isfile(DATA_FILE + ".gz"):
            self.qlearn.q_table = pandas.read_pickle(DATA_FILE + ".gz", compression = "gzip")
    ...
    def step(self, obs):
        ...
        if obs.first():
            ...
            self.cc_y, self.cc_x = (unit_type == _TERRAN_COMMAND_CENTER).nonzero()
        
        cc_y, cc_x = (unit_type == _TERRAN_COMMAND_CENTER).nonzero()
        ...
        if self.move_number == 0:
            ...
            if smart_action == ACTION_BUILD_SUPPLY_DEPOT:
                if supply_depot_count < 1 and _BUILD_SUPPLYDEPOT in obs.observation["available_actions"]:
                    if self.cc_y.any():
                ...
                if barracks_count < 1 and _BUILD_BARRACKS in obs.observation["available_actions"]:
                    if self.cc_y.any():
                ...
                if attack and _ATTACK_MINIMAP in obs.observation["available_actions"]:
